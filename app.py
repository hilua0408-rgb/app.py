...
with st.sidebar:
    ...
    # [cite_start]File uploader yahan hai [cite: 5]
...
# --- SUBTITLE PROCESSOR (Handles extraction and re-insertion) ---
    ...
    if not api_key or not uploaded_file:
        ...
        try:
            ...
            for i in range(0, total_lines, batch_sz):
                ...
                # [cite_start]--- 🤖 SYSTEM PROMPT (Added specifically as requested) --- 
...
1. Format MUST be:
   [ID]
   Translated Text
2. No merging IDs. No Markdown. No original text.
...
{batch_txt}"""
                ...
                while retry > 0:
                    try:
                        ...
                        # [cite_start]Extraction Logic [cite: 20]
                        matches = list(re.finditer(r'\[(\d+)\]\s*\n(.*?)(?=\n\[\d+\]|$)', resp.text, re.DOTALL))
                        if matches:
                            for m in matches: trans_map[m.group(1)] = m.group(2).strip()
                            [cite_start]success = True; break # [cite: 21]
                        else: raise Exception("Format Error (No matches found)")
                    except Exception as e:
                        st.error(f"⚠️ Error in Batch {current_batch_num}: {str(e)}")
                        retry -= 1
            ...
            if trans_map:
                ...
                [cite_start]st.download_button(label="⬇️ DOWNLOAD TRANSLATED FILE", data=out_content, file_name=f"translated_{uploaded_file.name}", use_container_width=True) # [cite: 24]
